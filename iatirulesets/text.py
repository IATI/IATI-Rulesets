        ...
        for case in cases:
            ...
            if 'paths' in case:
                for case_path in case['paths']:
                    ...
                    if show_all or simplify_xpath(case_path) == reduced_path:
                        ...
                        if rule == 'only_one':
                            ...
                            if other_paths:
                                out.append('``{0}`` must not be present if ``{1}`` are present.'.format(case_path, human_list(other_paths)))  # noqa
                        elif rule == 'atleast_one':
                            if other_paths:
                                out.append('Either ``{0}`` or ``{1}`` must be present.'.format(case_path, human_list(other_paths)))  # noqa
                            else:
                ...
                if show_all or simplify_xpath(case['less']) == reduced_path or simplify_xpath(case['more']) == reduced_path:
                    if case['less'] == 'NOW':
                        out.append('``{0}`` must not be in the past.\n\n'.format(case['more']))
                    elif case['more'] == 'NOW':
                        out.append('``{0}`` must not be in the future.\n\n'.format(case['less']))
                    else:
                        out.append('``{0}`` must be before or the same as ``{1}``\n\n'.format(case['less'], case['more']))
            else:
                print('Not implemented', case_path, rule, case['paths'])
    return out
